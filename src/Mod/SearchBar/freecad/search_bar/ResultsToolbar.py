...
def toolbarToolTip(nfo, setParent):
    ...
    in_workbenches = [
        "<li>"
        + (
            Serialize.iconToHTML(QtGui.QIcon(workbenches[wb].Icon))
        ...
        for the_toolbar in mw.findChildren(QtGui.QToolBar, toolbarName):
            ...
            for tbt in the_toolbar.findChildren(QtGui.QToolButton):
                ...
                if text != "":
                    ...
                    # The default action of a menu changes dynamically,
                    # instead of triggering the last action, just show the menu.
                    action = {
                        "handler": "tool",
                        "workbenches": toolbarIsInWorkbenches,
                        "toolbar": toolbarName,
