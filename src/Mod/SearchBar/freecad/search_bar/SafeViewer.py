...
class SafeViewer(QtGui.QWidget):
    """FreeCAD uses a modified version of QuarterWidget.

    The import pivy.quarter one will cause segfaults.  FreeCAD's FreeCADGui.createViewer()
    puts the viewer widget inside an MDI window, and detaching it without
    causing segfaults on exit is tricky.  This class contains some kludges to
    extract the viewer as a standalone widget and destroy it safely.
    """

    enabled = FreeCAD.ParamGet(
        "User parameter:BaseApp/Preferences/Mod/SearchBar"
    ).GetBool("PreviewEnabled", False)
    instances = []
    ...
    def __init__(self, parent=None):
        ...
        if SafeViewer.enabled:
            ...
            self.lbl_warning.setText(
                "Warning: the 3D preview has some stability issues. "
                "It can cause FreeCAD to crash (usually when quitting the application) "
                "and could in theory cause data loss, inside and outside of FreeCAD."
            )
            self.btn_enable_for_this_session = QtGui.QPushButton(
                "Enable 3D preview for this session"
            )
            ...
            self.btn_enable_for_future_sessions = QtGui.QPushButton(
                "Enable 3D preview for future sessions"
            )
