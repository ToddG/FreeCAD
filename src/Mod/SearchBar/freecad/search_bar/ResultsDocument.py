...
from freecad.search_bar import SafeViewer, SearchBox
...
# For some reason, the viewer always works except when used for two consecutive
# items in the search results: it then disappears after a short
# zoom-in+zoom-out animation.  I'm giving up on getting this viewer to work in
# a clean way, and will try swapping two instances so that the same one is
# never used twice in a row.  Also, in order to avoid segfaults when the module
# is reloaded (which causes the previous viewer to be garbage collected at some
# point), we're using a global property that will survive module reloads.
...
class DocumentObjectToolTipWidget(QtGui.QWidget):
    def __init__(self, nfo, setParent):
        ...
        if App._SearchBar3DViewer is None:
            ...
            # Tried setting the preview to a fixed size to prevent it from
            # disappearing when changing its contents, this sets it to a fixed
            # size but doesn't actually pick the size, .resize does that but
            # isn't enough to fix the bug.
            # safeViewerInstance.setSizePolicy(QtGui.QSizePolicy(QtGui.QSizePolicy.Preferred,
        ...
        # This is really a bad way to do this… to prevent the setExtraInfo
        # function from finalizing the object, we remove the parent ourselves.
        ...
        # hide before detaching, or we have widgets floating as their own
        # window that appear for a split second in some cases.
        #  oldParent.hide()         #  oldParent.setParent(None)
