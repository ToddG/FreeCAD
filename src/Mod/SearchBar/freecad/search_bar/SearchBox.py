...
from freecad.search_bar.SearchBoxLight import SearchBoxLight
...
class SearchBox(QtGui.QLineEdit):
    ...
    """
        ...
        # Will be initialized by calling getItemGroups() the first time the search
        # box gains focus, through focusInEvent and refreshItemGroups
        ...
        # Filtered model to which items are manually added. Store it as a
        # property of the object instead of a local variable, to prevent grbage collection.
        ...
        QtGui.QShortcut(
            QtGui.QKeySequence(QtCore.Qt.Key_Down), self, context=wdgctx
        ).activated.connect(self.listDown)
        QtGui.QShortcut(
            QtGui.QKeySequence(QtCore.Qt.Key_Up), self, context=wdgctx
        ).activated.connect(self.listUp)
        QtGui.QShortcut(
            QtGui.QKeySequence(QtCore.Qt.Key_PageDown), self, context=wdgctx
        ).activated.connect(self.listPageDown)
        QtGui.QShortcut(
            QtGui.QKeySequence(QtCore.Qt.Key_PageUp), self, context=wdgctx
        ).activated.connect(self.listPageUp)

        # Home and End do not work, for some reason.
        # QtGui.QShortcut(QtGui.QKeySequence.MoveToEndOfDocument, self, context = wdgctx)
        # .activated.connect(self.listEnd)
        # QtGui.QShortcut(QtGui.QKeySequence.MoveToStartOfDocument, self, context = wdgctx)
        # .activated.connect(self.listStart)
        # QtGui.QShortcut(QtGui.QKeySequence(QtCore.Qt.Key_End), self, context = wdgctx)
        # .activated.connect(self.listEnd)
        # QtGui.QShortcut(QtGui.QKeySequence('Home'), self, context = wdgctx).activated
        # .connect(self.listStart)

        QtGui.QShortcut(
            QtGui.QKeySequence(QtCore.Qt.Key_Enter), self, context=wdgctx
        ).activated.connect(self.listAccept)
        QtGui.QShortcut(
            QtGui.QKeySequence(QtCore.Qt.Key_Return), self, context=wdgctx
        ).activated.connect(self.listAccept)
        QtGui.QShortcut(
            QtGui.QKeySequence("Ctrl+Return"), self, context=wdgctx
        ).activated.connect(self.listAcceptToggle)
        QtGui.QShortcut(
            QtGui.QKeySequence("Ctrl+Enter"), self, context=wdgctx
        ).activated.connect(self.listAcceptToggle)
        QtGui.QShortcut(
            QtGui.QKeySequence("Ctrl+Space"), self, context=wdgctx
        ).activated.connect(self.listAcceptToggle)

        QtGui.QShortcut(
            QtGui.QKeySequence(QtCore.Qt.Key_Escape), self, context=wdgctx
        ).activated.connect(self.listCancel)

        # Initialize the model with the full list (assuming the text() is empty)
        # self.proxyFilterModel(self.text()) # This is done by refreshItemGroups on
        # focusInEvent, because the initial loading from cache can take time
    ...
    def proxyFocusInEvent(self, qFocusEvent):
        if self.firstShowList:
            ...
            mdl.appendRow(
                [
                    QtGui.QStandardItem(
                        genericToolIcon, "Please wait, loading results from cache…"
                    ),
    ...
    def proxyFilterModel(self, userInput):
        ...
        def filterGroup(group):
            if matches(group["text"]):
                # If a group matches, include the entire subtree
                # (might need to disable this if it causes too much noise)
                return group
            else:
        ...
        def addGroups(filteredGroups, depth=0):
            for group in filteredGroups:
                ...
                self.mdl.appendRow(
                    [
                        QtGui.QStandardItem(
                            group["icon"] or genericToolIcon, group["text"]
                        ),
    ...
    def setFloatingWidgetsGeometry(self):
        def getScreenPosition(widget):
            ...
            parentPos = (
                getScreenPosition(parent) if parent is not None else QtCore.QPoint(0, 0)
            )
        ...
        # TODO: this can still bump into the bottom of the screen,
        # in that case we should flip
    ...
    def proxyOnSelectionChanged(self, selected, deselected):
        ...
        if len(selected) > 0:
            index = selected[0]
            self.setExtraInfo(index)
            # Poor attempt to circumvent a glitch where the extra info
            # pane stays visible after pressing Return
    ...
    def setExtraInfo(self, index):
        if self.currentExtraInfo == (index.row(), index.column(), index.model()):
            # avoid useless updates of the extra info window;
            # this also prevents segfaults when the widget
            # is replaced when selecting an option from the
        ...
        if self.setExtraInfoIsActive:
            ...
            # print("lock") setExtraInfo can be called multiple times while
            # this function is running, so just before existing we check for
            # the latest pending call and execute it, if during that second
            # execution some other calls are made the latest of those will be
            # queued by the code a few lines above this one, and the loop will
            # continue processing until an iteration during which no further
            # call was made.

            while True:
                groupId = str(index.model().itemData(index.siblingAtColumn(2))[0])

                # TODO: move this outside of this class, probably use a single
                # metadata This is a hack to allow some widgets to set the
                # parent and recompute their size during their construction.
                ...
                def setParent(toolTipWidget):
                    ...
                    while w:
                        if hasattr(w.widget(), "finalizer"):
                            # The 3D viewer segfaults very easily if it is used
                            # after being destroyed, and some Python/C++
                            # interop seems to overzealously destroys some
                            # widgets, including this one, too soon?  Ensuring
                            # that we properly detacth the 3D viewer widget
                            # before discarding its parent seems to avoid these
                            # crashes.  print('FINALIZER')
                            w.widget().finalizer()
                        if w.widget() is not None:
                            # hide before detaching, or we have widgets
                            # floating as their own window that appear for a
                            # split second in some cases.
